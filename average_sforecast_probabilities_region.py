...
for m in np.arange(0,len(month_start)):
  print 'averaging fcst start-month '+str(month_start[m])+' & end-month '+str(month_end[m])
  p_file = dir_in+center_ids[model]+'_total_rainfall_'+p_name+'_probabilities_'+region+'_s'+mon_string(month_start[m])+'_e'+mon_string(month_end[m])+'_lead'+str(month_lead)+'.nc'
  ...
  # save as text file
  f_out = dir_in+center_ids[model]+'_total_rainfall_'+p_name+'_probabilities_avg_'+region+'_s'+mon_string(month_start[m])+'_e'+mon_string(month_end[m])+'_lead'+str(month_lead)+'.txt'
