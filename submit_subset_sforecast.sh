...
for model in $(seq 0 0); do
  for m in $(seq 0 2); do # loop through month_start/end
    ...
    #BSUB -n 1
    #BSUB -W 2:00
    python save_subset_sforecast.py $model ${month_start[$m]} ${month_end[$m]} $month_lead $region $lonmin $lonmax $latmin $latmax 2>> ${err_dir}err_save_${model}_${m}.log >> ${err_dir}out_save_${model}_${m}.log
    " >> job_save.sh
    bsub < job_save.sh
    rm job_save.sh
